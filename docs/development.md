...
複数のフォルダに分散している `.zxu` ファイルを、ローカルで `scripts/result/` に集約できます。スクリプトは `scripts/folders/` 以下を再帰的に検索し、通常ファイルのうち拡張子が `.zxu` のものだけを移動します。拡張子の大文字・小文字は区別しません。

プロジェクトのルートディレクトリから実行してください。

```bash
python scripts/extractZxu.py
```
...
```text
scripts/
├── folders/   # 入力フォルダ（.zxu 以外のファイルはそのまま）
├── result/    # 移動された .zxu ファイル
└── extractZxu.py
```

`scripts/result/` は実行時に作成されます。移動先に同名のファイルが存在する場合は上書きせずエラーで停止するため、既存ファイルを確認してから再実行してください。移動処理のため、必要に応じて事前に入力フォルダをバックアップしてください。
