...
`/analysis` を開いた時点では、ブラウザが取得済みのダイブログから次の確定的な指標とグラフだけを生成する。Microsoft Foundry は呼び出さない。

| 表示 | 算出規則 |
|---|---|
| 最深水深 | `dive_info.max_depth_m` が有限な正数であるログの最大値 |
| 最低水温 | 正の `location.water_min_temp_c`、未記録なら正の `location.surface_temp_c` を候補とした最小値 |
| 最長潜水時間 | `dive_info.dive_time_min` が有限な正数であるログの最大値 |
| エリア別潜水本数 | `location.name` の最初の半角 `:` または全角 `：` より前で集計。区切りなし、区切り前が空、名称なしは「不明」。本数上位8エリアを表示 |
...
エリア別潜水本数の棒をクリックすると、分析画面が取得済みの全ログから同じエリア抽出規則で対象を選び、グラフ直下にダイブログ一覧を展開する。再取得や画面遷移を挟まないため、半角・全角コロンやコロン前の空白にかかわらず、その棒の集計に含まれたログと一覧が必ず一致する。「不明」も同じ規則で一覧化し、各行からダイブログ詳細へ遷移できる。

- エリア別の本数、平均最大水深、平均潜水時間、平均最低水温、平均評価、評価済み本数
- 全体の同指標と月別本数
- 最大水深、潜水時間、最低水温は有限な正数だけを平均対象とし、`0` 以下は未記録として除外する
- 評価は `1` から `5` だけを平均対象とし、`0` や範囲外は除外する
- 無効な測定値や評価を持つログも、全体本数とエリア別本数には含める
- ダイブプロファイル時系列、認証情報、メールアドレスは送信しない

Foundry の応答は `backend/prompts/dive_analysis/response_schema.json` による厳格な Structured Outputs と、サーバー側の件数・文字数・エリア集合検証を両方通過した場合だけクライアントへ返す。分析用デプロイメントは `ANALYSIS_REPORT_AZURE_OPENAI_DEPLOYMENT` を最優先し、未設定時は共通のモデル解決規則へフォールバックする。本番では分析に `gpt-5.4`、GPS 提案に `gpt-4.1` を使用し、互いのモデル切り替えを分離している。
