...
認証ユーザーが所有する全ダイブログを集計し、Microsoft Foundry 上の GPT-5.4 デプロイメントで分析レポートを都度生成する。分析ページの「レポートの作り直し」を押したときだけ呼び出され、生成結果は保存しない。モデルにはプロファイル時系列や認証情報を送らず、エリア別・全体・評価の集計値のみを渡す。エリアはロケーション名の最初の `:` または `：` より前の文字列とし、区切りがない場合は「不明」とする。最大水深、最低水温、潜水時間が `0` 以下の値は未記録値として各指標の集計から除外する（ダイブ本数には含める）。
