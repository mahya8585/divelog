...
| `area` | string | ロケーション名の最初の `:` / `：` より前が完全一致するログでフィルタ。区切りなしは `不明` |
...
認証ユーザーが前回生成した保存済みレポートを返す。未生成の場合も `200 OK` で `report` は `null` となる。
...
認証ユーザーが所有する全ダイブログを集計し、Microsoft Foundry 上の GPT-5.4 デプロイメントで分析レポートを都度生成する。分析ページの「レポートの作り直し」を押したときだけ呼び出され、生成成功後に所有者の保存済みレポートを置き換える。モデルにはプロファイル時系列や認証情報を送らず、エリア別・全体・評価の集計値のみを渡す。エリアはロケーション名の最初の `:` または `：` より前の文字列とし、区切りがない場合は「不明」とする。最大水深、最低水温、潜水時間が `0` 以下の値は未記録値として各指標の集計から除外する（ダイブ本数には含める）。

保存先は Cosmos DB の `analysis_reports` コンテナで、パーティションキーは `/owner_email`、ID は `latest`。固定 ID の upsert と同一所有者パーティション内の余剰削除により、所有者ごとに最新1件だけを保持する。生成または保存に失敗した場合は既存レポートを維持する。

### 入力集計規則

| 集計項目 | 元フィールド | 採用条件 |
|---|---|---|
| エリア | `location.name` | 最初の `:` / `：` より前をtrim。区切りなし、空、名称なしは「不明」 |
| 最大水深 | `dive_info.max_depth_m` | 数値変換できる有限な正数のみ |
| 潜水時間 | `dive_info.dive_time_min` | 数値変換できる有限な正数のみ |
| 最低水温 | `location.water_min_temp_c` | 数値変換できる有限な正数のみ。AI集計では水面水温へフォールバックしない |
| 評価 | `dive_info.rating` | `1` 以上 `5` 以下のみ |
| 月 | `dive_info.datetime` | ISO 8601として解釈できる日時のみ |

測定値や評価が採用条件を満たさないログも、`total_dives` とエリア別の `dive_count` には含める。したがって平均値の母数と潜水本数の母数は一致しない場合がある。
...
リクエストボディは不要。呼び出し回数はユーザー/IPごとに `6 per hour` に制限される。
...
```json
{
  "area_trends": [
    {
      "area": "沖縄本島",
      "summary": "中深度で比較的長い潜水を繰り返す傾向があります。",
      "evidence": "42本、平均最大水深15.2m、平均潜水時間61分"
    }
  ],
  "user_trend": "年間を通じて中深度のロングダイブを好む傾向があります。",
  "recommendations": [
    {
      "spot": "パラオ",
      "country_or_region": "パラオ",
      "visited": false,
      "match_score": 4.7,
      "reason": "高評価だった温暖な中深度ダイブの傾向に合います。"
    }
  ],
  "generated_at": "2026-08-17T12:34:56+00:00",
  "model_deployment": "gpt-5.4"
}
```

`match_score` は AI が予測した適合度であり、実際のダイブ評価ではない。未訪問候補には国内・海外を含む。
...
|---|---|
| `401 Unauthorized` | トークンが無効または未指定 |
| `429 Too Many Requests` | 生成回数が上限を超過 |
| `503 Service Unavailable` | Foundry 接続未設定、モデル呼び出し失敗、または出力形式不正 |
