...
## 2026-08-18: ロケーションの緯度・経度編集が current_name エラーになる
...
ロケーション一覧の GPS 編集画面で緯度・経度を変更して保存すると、`current_name が不正です` と表示され、変更が保存されなかった。

### 原因

ロケーション名と GPS の一括更新対応で、API に更新前の表示名を表す `current_name` を必須項目として追加した一方、先に配信されていたフロントエンドはこの項目を送信しないため、フロントエンドとバックエンドのデプロイ世代がずれた期間にすべての更新が入力エラーになった。

### 修正対応

`current_name` を後方互換のため任意項目とし、省略された場合はパスの正規化名と一致する現在の表示名を、認証ユーザーが所有するダイブログから API 側で解決するようにした。指定された場合は従来どおりパスとの整合性を検証する。

### 長期修正計画とその進捗

- **完了**: 旧フロントエンド相当の `current_name` を含まないリクエストでも、対象名と変更後の緯度・経度が更新処理へ渡ることを自動検証した。
- **完了**: 名前解決を認証ユーザー所有のダイブログに限定し、他ユーザーのロケーションを更新対象にしない。
- **運用**: フロントエンドと API のリクエスト項目を変更するときは、段階デプロイ中も旧クライアントを受け付ける移行期間を設ける。

---

## 2026-08-18: 保存済み分析レポートの表示後にグラフが空白になる
...
分析ページを再読み込みすると保存済みAI分析レポートは表示されるが、「エリア別潜水本数」と「月別潜水本数」のグラフ領域がどちらも空白になった。

### 原因

分析ページは `loading` が `true` の間、`v-if` によりグラフの `canvas` をDOMへ描画しない。一方、保存済みレポートを取得した後も `loading` を解除する前に `nextTick()` とChart.jsの生成処理を実行していたため、両方のcanvas参照が `null` のままになり、グラフ生成がスキップされた。

### 修正対応

ダイブログと保存済みレポートの取得完了後に `loading` を解除し、その次のDOM更新を `nextTick()` で待ってから集計とChart.jsの生成を実行するよう、分析ページの初期化順序を修正した。

### 長期修正計画とその進捗

- **完了**: Chart.jsの生成前に、グラフのcanvasがDOMへ描画される初期化順序へ統一した。
- **完了**: フロントエンドの本番ビルドで分析ページとChart.jsに関するエラーがないことを確認した。
- **運用**: ローディング表示配下のDOM参照を使う処理を追加する場合は、表示条件を切り替えた後の `nextTick()` で参照可能性を確認する。
